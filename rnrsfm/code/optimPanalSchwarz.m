...
options = optimset('Display','none','TolFun',1e-6,'MaxIter',20,'Jacobian','on','UseParallel',true);
%options = optimset('Display','on','TolFun',1e-6,'MaxIter',100,'Jacobian','on');
cf = @(ctrlpts)L_cf(bbs,ctrlpts,Q,Qp,SQ,er,l,options);
...
function [y,jac] = L_cf(bbs,ctrlpts,Q,Qp,SQ,er,l,options)
...
jacs=er.*[jI;jJ;jM;jN];
jac=[jacd;jacs];
%  JacobPatter=abs(jac)>0;
%  options.JacobPattern=JacobPatter;
%disp(sprintf('%f\n',y'*y));
end
